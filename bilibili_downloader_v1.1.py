...
def out_mkv(bilivideo , location , page):
    info = sync(bilivideo.get_info())
    name = info['title']
    if(page != 0):
        name = name + ' (P' + str(page) + '. '  + info['pages'][page - 1]['part'] + ')'
    ...
    os.remove(location + name + '.cmt.xml')
    os.rename(location + 'out' + '.mkv',location + name + '.mkv')
    if (message == 0):
        return 0
    else:
        return 1
...
def video_download(bv , location , page):
    if(page != 0):
        order = 'https://www.bilibili.com/video/' + bv + '?p=' + str(page)
    else:
...
def danmaku_to_ass(bilivideo , location , page):
    ...
    dms = sync(bilivideo.get_danmakus(page_index = page))
...
p = sync(v.get_info())['videos']
if(p != 1):
    page = input('此视频含有多个分p,请输入想下载的分p：')
    page = int(page)
else:
    page = 0
...
video_download(bv = bv, location = location,page = page)
danmaku_to_ass(bilivideo = v,location = location,page = page - 1 if page != 0 else 0)
out_mkv(bilivideo = v,location = location,page = page)
while(1):
    cnt = 1
